	...
	{ // symbol is a reserved word or an identifier.
		...
		while(ch == '['){
			...
			if(ch!=']')
				error(27);
			getch();
			if (arr_index_len > MAXNUMLEN)
				error(25);     // The number is too great.
			if(num_dim >= MAXDIM)
				error(28);     // Too many dimensions for the array.
			len[num_dim] = num;
			num_dim++;
		}
...
void factor(symset fsys)
{
	...
	{
		if (sym == SYM_IDENTIFIER || sym == SYM_ARRAY || sym == SYM_TIMES)  //ident, array or pointer
		{
...
void block(symset fsys)
{
	...
	do
	{
		...
		set1 = createset(SYM_IDENTIFIER, SYM_NULL);// allow multiple declarations here
		set = uniteset(statbegsys, set1);
